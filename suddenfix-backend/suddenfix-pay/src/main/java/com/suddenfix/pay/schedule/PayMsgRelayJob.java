package com.suddenfix.pay.schedule;
...
import static com.suddenfix.common.enums.RedisPreMessage.PAY_SCHEDULE;

@Slf4j
@Component
@RequiredArgsConstructor
public class PayMsgRelayJob {
    ...
    public void relayMessages() {
        RLock lock = redissonClient.getLock(PAY_SCHEDULE.getValue());
        boolean locked = false;
        try {
            locked = lock.tryLock(0, 10, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            log.warn("【支付服务】本地消息投递任务获取分布式锁被中断");
            return;
        }
        if (!locked) {
            return;
        ...
        try {
            ...
            log.info("【支付服务】定时任务启动，扫描到 {} 条待发送消息", msgList.size());
                ...
                try {
                    ...
                    correlationData.getFuture().whenComplete((confirm, ex) -> {
                        if (ex == null && confirm != null && confirm.isAck()) {
                            log.info("【支付服务】本地消息投递成功，msgId={}, topic={}", msg.getMsgId(), msg.getTopic());
                        ...
                        log.warn("【支付服务】本地消息投递失败，msgId={}, topic={}, nextRetryTime={}, reason={}",
                    ...
                    Date nextRelayTime = new Date(System.currentTimeMillis() + (long) (msg.getRetryCount() + 1) * RELAY_TIME);
                    log.warn("【支付服务】本地消息发送异常，msgId={}, topic={}, nextRetryTime={}",
                            msg.getMsgId(), msg.getTopic(), nextRelayTime, ex);
                    msgMapper.updateMsgRelay(msg.getMsgId(),msg.getBusinessId(),nextRelayTime);
                }
            }
        }catch (Exception e){
            log.error("【支付服务】执行消息轮询任务发生异常", e);
        }finally {
