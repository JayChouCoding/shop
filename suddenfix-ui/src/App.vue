...
import { clearSession, hasSession, sessionState } from './stores/session';

const route = useRoute();
const router = useRouter();

const navItems = [
  { label: '商品会场', to: '/products' },
  { label: '购物车', to: '/cart' },
...
<template>
  <div class="app-shell">
    <header class="app-topbar">
      <div class="brand-block" @click="router.push('/products')">
        <span class="brand-badge">SF</span>
        <div>
          <div class="brand-title">SuddenFix</div>
          <div class="brand-subtitle">高并发秒杀与连贯支付体验</div>
        </div>
      </div>
      ...
      <div class="top-actions">
        <el-tag v-if="isAuthed" class="account-pill" effect="dark" type="success">
          {{ sessionState.account || '已登录用户' }}
        </el-tag>
